...
$MyInvocation.MyCommand|Format-List * |Out-Host
Get-PSCallStack
...
Get-ChildItem ENV:*|Sort-Object Name | %{ Write-Host "$($_.Name)=``$($_.Value)``" }
Get-Alias|% {write-host "$($_.Name) $($_.Definition)"}

Write-Host "User: $($env:USERNAME)/$($env:USERDOMAIN)"
Write-Host "User Context: $($env:ChocolateyUserContext)"
$env:ChocolateyDomainContext =$(Get-WmiObject win32_loggedonuser |Select-Object Antecedent -Unique | `
  Select-String -Pattern "Domain=""([^""]+)"".*Name=""$($env:ChocolateyUserContext)""" ).Matches.Groups[1].Value
Write-Host "Domain Context: $($env:ChocolateyDomainContext)"
...
$packageArgs = @{
  ...
  softwareName  = 'example*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
